...
local function ActionLayerChanged(event, layerIndex, activeLayerIndex)
	data.inDialog = activeLayerIndex > 2
end
...
do
	...
	local function OnMapPing(pinType)
		if pinType == MAP_PIN_TYPE_PLAYER_WAYPOINT then
			...
			state = state - 1
			if state <= 0 then
				LMP:UnregisterCallback("AfterPingAdded", OnMapPing)
...
local function HookInteraction()
	...
	data.reelIn:SetAnchor(CENTER, RETICLE.interactKeybindButton, LEFT, 16, 0)
...
local function LayoutPins(pinManager)
	...
	local offsetX, scaleX, offsetY, scaleY = measurements.offsetX, 1 / measurements.scaleX, measurements.offsetY, 1 / measurements.scaleY
	local pinTypeId, GetGameTimeSeconds = data.pinTypeId, GetGameTimeSeconds
	...
	local function DrawPins(map, identifier)
		...
		local function DrawPin()
			local start, x, y, pin = GetFrameTimeSeconds()
			local spendTime = WORLD_MAP_FRAGMENT:IsShowing() and math.min(0.032, math.max(0.014, .5 / GetFramerate())) or 0.010
			while lure do
				x, y = (lure.x - offsetX) * scaleX, (lure.y - offsetY) * scaleY
				if x > 0 and x < 1 and y > 0 and y < 1 then
					pin = pinManager:CreatePin(pinTypeId, FishingHole:new({x = x, y = y, lure = lure}), x, y)
				...
				if (GetGameTimeSeconds() - start) >= spendTime then
					break
				end
			end
			if not lure then
