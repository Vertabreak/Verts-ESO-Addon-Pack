...
mkstr(SI.TELE_CHAT_WHISPER_NOTE, "注意！あなたはオフラインになっており、すべてのささやきメッセージを受信できません！")
...
mkstr(SI.TELE_SETTINGS_WHISPER_NOTE, "ささやき警告の表示")
mkstr(SI.TELE_SETTINGS_WHISPER_NOTE_TOOLTIP, "オフラインに設定して誰かにささやくと、あなたがささやきメッセージを受信できないという注意がチャットに表示されます。この機能は、返答への不本意なブロックを回避するのに役立ちます。")
...
mkstr(SI.TELE_SETTINGS_PRIORITIZATION_DESCRIPTION, "テレポート先として利用するプレイヤーの優先度を設定できます。ギルドへの加入と脱退のあとにはリロードが必要です。 |ca20000この項目はキャラクター毎に設定されます(グローバル設定ではありません)！|r")
mkstr(SI.TELE_SETTINGS_SCAN_BANK_FOR_MAPS, "銀行内の地図も検索する")
mkstr(SI.TELE_SETTINGS_SCAN_BANK_FOR_MAPS_TOOLTIP, "銀行にある地図についても［宝の地図・調査報告・手掛かり］の一覧に表示します。")
...
mkstr(SI.TELE_SETTINGS_SCAN_LEADS, "占い可能な手掛かりを表示")
mkstr(SI.TELE_SETTINGS_SCAN_LEADS_TOOLTIP, "宝の地図や調査報告と一緒に占い可能な手掛かりを表示します。")
