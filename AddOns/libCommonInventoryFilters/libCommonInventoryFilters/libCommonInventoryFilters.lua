--2020-11: LibCommonInventoryFilters (libCIF) is obsolete and should not be used anymore. Please deinstall it
local myNAME, myVERSION = "libCommonInventoryFilters", 1.61
LibCIF = LibCIF or {}
...
local function showSearchBoxes()
    ...
    local width = searchBoxes[1]:GetWidth()
...
local function OnAddonLoaded(eventType, addonName)
    ...
    for _, inventory in pairs(PLAYER_INVENTORY.inventories) do
        local searchBox = inventory.searchBox:GetParent()
        searchBoxes[#searchBoxes + 1] = searchBox
    end
