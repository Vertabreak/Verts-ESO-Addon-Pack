...
function CS.DrawCharacters()
  ...
  local function GetResearch(char,nr)
	...
	if CS.Account.options.overviewstyle ~= 3 then	
		for craft,craftData in pairs(CS.Data.crafting.researched[char]) do	
			...
			if current == maxsim or unknown == 0 then simcolor = '|c00FF00' else simcolor = '|cFF0000' end
  ...
  for x = 1,18 do control = WM:GetControlByName('CraftStoreFixed_CharacterFrame'..x); if control then control:SetHidden(true) end end
  for nr, char in pairs(CS.GetCharacters()) do
	...
	-- fallback race
	if CS.Races[player.race] == nil then player.race = 1 end	
	...
	if player.skillPoints == nil then player.skillPoints = "?/?" end	
	...
	if player.skyShards == nil then player.skyShards = "?/?" end	
...
function CS.ScanBagResearch(target_craft,target_line,target_trait,bag,secondary)
  ...
  if not secondary then secondary = false end
  ...
  for _, data in pairs(bag) do
    ...
    if target_craft == craft and target_line == line and target_trait == trait then
		...
		if locked or (secondary and quality > ITEM_QUALITY_ARCANE) then found = false end
		if found then return data.bagId, data.slotIndex end
	end
  end
  return false
end
...
function CS.UpdateBag()
  ...
  for _, data in pairs(bag) do
    ...
    if data.bagId == BAG_BANK or data.bagId == BAG_SUBSCRIBER_BANK then bagName = CS.Lang.en.bank end
    if IsHouseBankBag(data.bagId) then bagName = CS.Lang.en.housebank..(data.bagId-7) end
    if data.bagId == BAG_VIRTUAL then bagName = CS.Lang.en.craftbag end	
...
function CS.IsLocked(bagId,slotIndex)
	...
	if not locked and FCOIS and FCOIS.addonVars.gPlayerActivated then
		--Try to prevent extraction
		if isGlyph then
			locked = FCOIS.IsEnchantingExtractionLocked(bagId,slotIndex)
...
function CS.Queue()
  if CS.Init then
    ...
    if CS.Account.options.timeralarm ~= 4 or CS.Account.options.mountalarm ~= 4 or CS.Account.options.researchalarm ~= 4 then
...
function CS.UpdateScreen()
  ...
  for id,value in pairs(CS.Data.furnisher.knowledge[CS.SelectedPlayer]) do
    for x,recipe in pairs(CS.Furnisher.recipe) do
	  if recipe.id == id then CS.Furnisher.recipe[x].known = value; break end
    end
  end	
  end
  CS.UpdateAllStudies()
...
local function NeedAppend(need,unneed,researching)
	if researching == nil then researching = {} end
...
function CS.UpdateStyleKnowledge(activate)
	...
	for id,data in pairs(styleNames) do
		...
		for chapter = 1,14 do
			...
			if known then control:SetColor(1,1,1,1) else control:SetColor(1,0,0,0.5) end
		end
		CS.FilterStyles()
	end
...
function CS.UpdateIngredientTracking()
	for k in next,CS.Cook.ingredient do CS.Cook.ingredient[k] = nil end
...
function CS.UpdateResearchWindows()
  ...
  for craft,craftData in pairs(CS.Data.crafting.researched[CS.SelectedPlayer]) do
    ...
    local simcolor, unknowncolor, current = '|cFFFFFF', '|cFF0000', row - 1
    if current == maxsim or unknown == 0 then simcolor = '|c00FF00' else simcolor = '|cFF0000' end
	if unknown == 0 then unknowncolor = '|c00FF00' end
...
function CS.AddResearchItems(items,bag_data)
	if not bag_data then bag_data = SHARED_INVENTORY:GenerateFullSlotData(nil,BAG_WORN,BAG_BACKPACK,BAG_BANK,BAG_SUBSCRIBER_BANK) end
	for index,data in pairs (bag_data) do
		if next(items) == nil then break end
...
function CS.UpdateStored(action,data,replace)
  ...
  local function CompareItem(craft,line,trait,q1,l1,v1)
	...
	if CS.NilCheck(CS.Account.crafting.stored,{},craft,line,trait) == {} then return true
	elseif not CS.Account.crafting.stored[craft][line][trait].link then return true 
	else
  ...
  if craft and line and trait then
    if action == 'added' then
      if data.bagId == BAG_BANK or data.bagId == BAG_SUBSCRIBER_BANK then owner = CS.Lang.en.bank end
	  if IsHouseBankBag(data.bagId) then owner = CS.Lang.en.housebank end
      if data.bagId == BAG_GUILDBANK then owner = CS.Lang.en.guildbank end
      if data.bagId == BAG_VIRTUAL then owner = CS.Lang.en.craftbag end
...
function CS.CloseRecipeWindow()
	...
	for x = 1,CraftStoreFixed_RecipePanelScrollChild:GetNumChildren() do CS.HideControl('CraftStoreFixed_RecipePanelScrollChildButton'..x) end
end
...
function CS.RecipeShow(id,inc)
  ...
  if CS.Cook.recipe[id].known then color = CS.Quality[CS.Cook.recipe[id].quality] else color = {1,0,0,1} end
...
function CS.RecipeShowCategory(list)
  if list == nil or list > 16 then list = 1 end
...
function CS.CookStart(control, button, isEnchanting)
  ...
  if isEnchanting == nil then isEnchanting = false end
  ...
  if button == 3 then
	...
	if CS.Character.favorites[tradeType][idx] then CS.Character.favorites[tradeType][idx] = nil
	else CS.Character.favorites[tradeType][idx] = {control.data.list, control.data.id} end
...
function CS.CookShowRecipe(control,list,id,inc,sound,enchanting)
  ...
  if known then
    ...
    for num = 1, numIngredients do
      ...
      if count < qtyReq then color = 'FF0000'; fault = true else color = '00FF00' end
      if (count/qtyReq) < maxval then maxval = math.floor(count/qtyReq) end
	...
	if enchanting then
		...
		if CS.Character.favorites[CRAFTING_TYPE_ENCHANTING][list..'_'..id] then mark = '|t16:16:esoui/art/characterwindow/equipmentbonusicon_full.dds|t ' else mark = '' end
	else		
		if CS.Character.favorites[CRAFTING_TYPE_PROVISIONING][list..'_'..id] then mark = '|t16:16:esoui/art/characterwindow/equipmentbonusicon_full.dds|t ' else mark = '' end
	end	
	...
	if search ~= '' and search ~= (GetString(SI_GAMEPAD_HELP_SEARCH) .. "...") and override == false then
		...
		if list == 17 then
			...
			for _,val in pairs(CS.Character.favorites[CRAFTING_TYPE_PROVISIONING]) do
				...
				if inc > CraftStoreFixed_CookFoodSectionScrollChild:GetNumChildren() then break end
			end
...
function CS.RuneCreate(control,button)
  ...
  if button == 3 then
	...
	if CS.Character.favorites[CRAFTING_TYPE_ENCHANTING][idx] then CS.Character.favorites[CRAFTING_TYPE_ENCHANTING][idx] = nil
	else CS.Character.favorites[CRAFTING_TYPE_ENCHANTING][idx] = { id, control.data.level, control.data.quality, control.data.essence, control.data.potencyType } end
		...
		if (item == ITEMTYPE_GLYPH_ARMOR or item == ITEMTYPE_GLYPH_WEAPON or item == ITEMTYPE_GLYPH_JEWELRY) and not CS.IsLocked(data.bagId, data.slotIndex) then
			...
			if glyph_position == 0 then
				...
				if not level and not cplevel then
					...
					if not level then level = cplevel+50 end
...
function CS.RuneSetValue(key,value,ptype)
  ...
  elseif key == 13 then CS.Character.runemode = 'furniturefavorites'
  end
end
...
function CS.RuneShow(nr,id,quality,level,essence,potencytype)
  ...
  if CS.Character.favorites[CRAFTING_TYPE_ENCHANTING][id..'_'..quality..'_'..level] then mark = '|t16:16:esoui/art/characterwindow/equipmentbonusicon_full.dds|t ' else mark = '' end
...
function CS.RuneShowMode(atStationOnly)
    if atStationOnly == nil then atStationOnly = false end
    if (atStationOnly and not CS.Extern) or not atStationOnly then
        ...
        elseif CS.Character.runemode == 'furniturefavorites' then CS.RuneShowFurnitureFavorites()
...
function CS.RuneShowFurnitureFavorites()
	...
	for _,val in pairs(CS.Character.favorites[CRAFTING_TYPE_ENCHANTING]) do
		...
		if inc > CraftStoreFixed_RuneGlyphSectionScrollChild:GetNumChildren() then break end
	end		
	CraftStoreFixed_RuneGlyphSectionScrollChild:SetHeight(inc * 30 + 20)
end
...
function CS.SetItemMark(control,linksource)
  ...
  if CS.Account.options.markitems then
    ...
    if item == ITEMTYPE_INGREDIENT then
      ...
      if ingid then 
		if CS.Cook.ingredient[ingid] then Show(mark,'esoui/art/inventory/newitem_icon.dds',{0,1,0});
		elseif mark then
			mark:SetHidden(true)
		end 
		return
    ...
    if item == ITEMTYPE_RECIPE then
	  ...
	  specializedItemType == SPECIALIZED_ITEMTYPE_RECIPE_JEWELRYCRAFTING_SKETCH_FURNISHING then
        if CS.IsBlueprintNeeded(link) ~= '' then Show(mark,'esoui/art/inventory/newitem_icon.dds',SELF); return end
      else
...
function CS.HideCrownStyles(init)
  local val, tex = {[true]='checked',[false]='unchecked'}, '|t16:16:esoui/art/buttons/checkbox_<<1>>.dds|t |t2:2:x.dds|t '
  if not init then CS.Character.hidecrownstyles = not CS.Character.hidecrownstyles end
...
function CS.FilterStyles()
  ...
  local filterCrown = CS.Character.hidecrownstyles
    ...
    local style = GetValidItemStyleId(data.id)
      ...
      if (filterPerfected and CS.Style.IsPerfectedStyle(style))
	  or (filterCrown and CS.Style.IsCrownStyle(style))
      or (filterSimple and CS.Style.IsSimpleStyle(style)) then
        c:SetHidden(true)
        c:SetHeight(h[true])
      else
        c:SetHidden(false)
...
function CS.HideKnownBlueprints(init)
	local val, tex = {[true]='checked',[false]='unchecked'}, '|t16:16:esoui/art/buttons/checkbox_<<1>>.dds|t |t2:2:x.dds|t '
	if not init then CS.Character.hideKnownBlueprints = not CS.Character.hideKnownBlueprints end
...
function CS.HideUnknownBlueprints(init)
	local val, tex = {[true]='checked',[false]='unchecked'}, '|t16:16:esoui/art/buttons/checkbox_<<1>>.dds|t |t2:2:x.dds|t '
	if not init then CS.Character.hideUnknownBlueprints = not CS.Character.hideUnknownBlueprints end
...
function CS.HideKnownRecipes(init)
	local val, tex = {[true]='checked',[false]='unchecked'}, '|t16:16:esoui/art/buttons/checkbox_<<1>>.dds|t |t2:2:x.dds|t '
	if not init then CS.Character.hideKnownRecipes = not CS.Character.hideKnownRecipes end
...
function CS.HideUnknownRecipes(init)
	local val, tex = {[true]='checked',[false]='unchecked'}, '|t16:16:esoui/art/buttons/checkbox_<<1>>.dds|t |t2:2:x.dds|t '
	if not init then CS.Character.hideUnknownRecipes = not CS.Character.hideUnknownRecipes end
...
function CS.GetTimer()
  ...
  local crafts = {CRAFTING_TYPE_BLACKSMITHING,CRAFTING_TYPE_CLOTHIER,CRAFTING_TYPE_WOODWORKING,CRAFTING_TYPE_JEWELRYCRAFTING}
  for _, char in pairs(CS.GetCharacters()) do
    if CS.Account.player[char].mount.time > 1 then table.insert(TIMER,{info = ZOSF(CS.Loc.finishMount, char), id = '$M' .. char, time = CS.Account.player[char].mount.time}) end
    for _,craft in pairs(crafts) do
      for line = 1, GetNumSmithingResearchLines(craft) do
        for trait = 1, CS.MaxTraits do
...
function CS.GetTrait(link)
  ...
  if trait==ITEM_TRAIT_TYPE_ARMOR_NIRNHONED then trait=19 end -- Nirnhoned Weapon replacement
  if trait==ITEM_TRAIT_TYPE_WEAPON_NIRNHONED then trait=9 end -- Nirnhoned Armor replacement
...
function CS.GetItemQuantity(link,storage)
	if not storage then storage = false end
	local qty = {}
      ...
      for z,trait in pairs(line)do
        if trait.id == id then
          for char,data in pairs(CS.Data.crafting.researched)do
            if CS.Data.crafting.studies[char][x][y][z] and not data[x][y][z] then return true end
          end
        end
      end
    end
...
function CS.OptionSetSelect(control,button)
  if button == 2 then
    ...
    for x = 1,3 do
      ...
      elseif CS.Sets[nr].nodes[x] == -2 then nodename = CS.Loc.TT[18]
	  -- todo: add explanation of location
	  elseif CS.Sets[nr].nodes[x] == -3 then nodename = '' end
...
function CS.CharacterInitialize()
  -- safety check due to Dragonhold quests
  if CraftStoreFixed_CharacterFrame1 ~= nil then return end
  ...
  for x,char in pairs(CS.GetCharacters()) do
	...
	if CS.Account.options.overviewstyle ~= 3 then
		for y,z in pairs(skills) do 
		  ...
		  btn:SetHandler('OnMouseExit', function(self) CS.Tooltip(self,false) end)
		  xpos = xpos + 141
		  if y == 4 then xpos = 0; ypos = ypos+29; end
		end 
	end
...
function CS.TooltipShow(control,link,id)
  local function localizeStorage(storage)
	...
	if GetCVar('language.2') ~= 'en' then
		...
		if storage == CS.Lang.en.bank then localize = CS.Loc.bank end
		if storage:find(CS.Lang.en.housebank) then localize = storage:gsub(CS.Lang.en.housebank,CS.Loc.housebank) end
		if storage == CS.Lang.en.guildbank then localize = CS.Loc.guildbank end
		if storage == CS.Lang.en.craftbag then localize = CS.Loc.craftbag end
	end
	if localize == "" then
		localize = storage
	end	
  ...
  if it == ITEMTYPE_RACIAL_STYLE_MOTIF then need, unneed = CS.IsStyleNeeded(link)
  ...
  elseif it == ITEMTYPE_ENCHANTING_RUNE_POTENCY then 
	if CS.Account.options.displayrunelevel then need = CS.IsPotency(link) end
  elseif CS.IsValidEquip(GetItemLinkEquipType(link)) then
    local craft,line,trait = CS.GetTrait(link)
    ...
    if craft and line and trait then need, unneed = CS.IsItemNeeded(craft,line,trait,id,link) end
  end
  if need ~= '' and CS.Account.options.displayunknown then control:AddLine(need,'CraftStoreFixedFont') end
  if unneed ~= '' and CS.Account.options.displayknown then control:AddLine(unneed,'CraftStoreFixedFont') end
  if CS.Account.options.showstock and CS.Account.storage[stripedLink] then
    if CS.Account.materials[stripedLink] and CS.Account.storage[CS.Account.materials[stripedLink].link] then
      ...
      for x, stock in pairs(CS.Account.storage[stripedLink]) do if stock and stock > 0 then table.insert(store,'|c8085FF'..localizeStorage(x)..':|r |cC0C5FF'..stock..'|r') end end
      if #store > 0 then control:AddLine(prefix1..table.concat(store,', '),'CraftStoreFixedFont') end
      store={}
      for x, stock in pairs(CS.Account.storage[pairedInfo.link]) do if stock and stock > 0 then table.insert(store,'|c8085FF'..localizeStorage(x)..':|r |cC0C5FF'..stock..'|r') end end
...
function CS.CloseBlueprintWindow()
	...
	for x = 1,CraftStoreFixed_BlueprintPanelScrollChild:GetNumChildren() do CS.HideControl('CraftStoreFixed_BlueprintPanelScrollChildButton'..x) end
end
...
function CS.BlueprintMark(control,button)
  ...
  elseif button == 1 and not FurPreview then CS.Chat:Print(CS.Loc.noItemPreview)
  elseif button == 2 then CS.ToChat(control.data.link)
  else
...
function CS.BlueprintShowCategory(list)
  if list == nil or list > 7 then list = 1 end
  local inc, known, total = 1, 0, 0
  for x = 1,CraftStoreFixed_BlueprintPanelScrollChild:GetNumChildren() do CS.HideControl('CraftStoreFixed_BlueprintPanelScrollChildButton'..x) end
