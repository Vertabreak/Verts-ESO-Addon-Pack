...
local function RememberSelectedPanel(panel)
	if
		panel ~= nil and panel.data ~= nil and panel.data.type == "panel" and
			LAM2:GetAddonSettingsFragment():GetState() == SCENE_FRAGMENT_SHOWN
	 then
...
function addon:SetupControls(panel)
	...
	if GetESOFullVersionString then
		local year, month, day, hour, min, sec, platform =
			GetESOFullVersionString():match("%s(%d+)/(%d+)/(%d+)%s(%d+):(%d+):(%d+)%s%(%d+.(%l+)")
